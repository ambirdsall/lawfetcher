...
//   1) `require('../types/citation')` is used to hardcode the obvious
//      dependency on `Citation` in `detectType.js` AND
//      `require('../functions/detectType')` is used in `citation.js`, which is
//      a circular dependency that breaks stuff.
...
//      inserted into its own test file via a `require` call, and the
//      `Citation.from` definition prefills the `taxonomy` argument, meaning it
//      cannot be tested as such with any controls for the actual citation
//      `type_list`.
...
module.exports = curry(function detectType(citationConstructor, taxonomy, citationText) {
